...
class AVDBottleneckBlock(BottleneckBlock):
    def __init__(
        ...
        *,
        ...
        if self.radix > 1:
            from cvpods.backbone.backbone.splat import SplAtConv2d
...
class ResNet(Backbone):
    def __init__(self, stem, stages, num_classes=None, out_features=None, zero_init_residual=False, **kwargs):
        """
        Args:
        ...
        self.transforms = {}
        last_channels = self.stem.out_channels
        for i, blocks in enumerate(stages):
            ...
            if "tf_stages" in kwargs:
                if name in kwargs["tf_stages"]:
                    num_groups = kwargs["tf_num_groups"]
                    tf = TreeFilterV2(last_channels, curr_channels, 
                                      num_groups, num_groups=num_groups)
                    self.add_module(name + "_tf", tf)
                    self.transforms[name] = tf
            last_channels = curr_channels
    ...
    def forward(self, x):
        ...
        for stage, name in self.stages_and_names:
            curr_features = stage(x)
            if name in self.transforms:
                transform = self.transforms[name]
                if isinstance(transform, TreeFilterV2):
                    curr_features = transform(curr_features, x)
            x = curr_features
...
def build_resnet_backbone(cfg, input_shape):
    ...
    tf_stages = cfg.MODEL.RESNETS.TF_STAGES
    tf_num_groups = cfg.MODEL.RESNETS.TF_NUM_GROUPS
    # fmt: on
    ...
    if not stride_in_1x1 and "MSRA" in cfg.MODEL.WEIGHTS:
        logger.warning("Using pretrain weight from MSRA with MODEL.RESNETS.STRIDE_IN_1X1 == False")
    elif stride_in_1x1 and "MSRA" not in cfg.MODEL.WEIGHTS and cfg.MODEL.WEIGHTS:
        logger.warning(
            "Using pretrain weight not from MSRA with MODEL.RESNETS.STRIDE_IN_1X1 == True"
        )
