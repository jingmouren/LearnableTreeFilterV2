...
class TreeFilterV2(nn.Module):
    def __init__(self, guide_channels, in_channels, embed_channels, num_groups=1, eps=1e-8):
        ...
        ''' Embedding Layers '''
        self.embed_layer = nn.Conv2d(in_channels, embed_channels, kernel_size=1)
        self.conf_layer  = nn.Conv2d(in_channels, num_groups, kernel_size=1)
        self.guide_layer = nn.Conv2d(guide_channels, self.embed_channels, kernel_size=1)
